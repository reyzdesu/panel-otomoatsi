...
app.post('/api/order', async (req, res) => {
  ...
  if (!products[productKey]) return res.status(400).json({ success: false, msg: "Produk tidak valid" })
  ...
  try {
    const payRes = await fetch("https://app.pakasir.com/api/transactioncreate/qris", {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ project: process.env.pakasirSlug, order_id: orderId, amount, api_key: process.env.pakasirApiKey })
    })
    const payData = await payRes.json()
    if (!payData.payment) throw new Error("Gateway error")
    const tx = { username, productKey, amount, orderId, status: 'pending', qrRaw: payData.payment.payment_number, productName: products[productKey].name, createdAt: Date.now() }
    db.set(orderId, tx)
